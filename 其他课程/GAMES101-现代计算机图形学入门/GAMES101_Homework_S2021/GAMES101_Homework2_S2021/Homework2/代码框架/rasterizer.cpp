...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    float mx = std::min(v[0].x(), std::min(v[1].x(), v[2].x()));
    float my = std::min(v[0].y(), std::min(v[1].y(), v[2].y()));
    float Mx = std::max(v[0].x(), std::max(v[1].x(), v[2].x()));
    float My = std::max(v[0].y(), std::max(v[1].y(), v[2].y()));
