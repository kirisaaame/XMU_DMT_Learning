...
public class Menu {
    ...
    public static void NewCar(Scanner scanner) {
        while (true) {
            ...
            if (parts.length < 5) {
                System.out.println("输入数据短于要求，请重新输入或输入end结束：");
    ...
    public static void FindCar(Scanner scanner) {
        while (true) {
            ...
            if (vehicleType.equals("小汽车") || vehicleType.equals("卡车") || vehicleType.equals("null")) {
                List<Vehicle> matching = new ArrayList<>(); // 符合条件的小汽车列表
                for (Vehicle vehicle : vehicles) {
                    if (vehicle instanceof Car car) { // 检查是否是小汽车
                    ...
                    } else if (vehicle instanceof Truck truck) { // 检查是否是卡车
                ...
                if (count == 0) {
                    ...
                    for (Vehicle vehicle1 : matching) {
                        vehicle1.PrintInfo();
                    }
                }
            } else {
                System.out.println("输入的车辆类型不正确，请重新输入");
            }
        }
    }

    public static void PrintCar(Scanner scanner) {
